...
function MetricCard({ label, value, icon, loading, sublabel }: MetricCardProps) {
  return (
    <Paper sx={{ p: 3, flex: "1 1 200px", minWidth: 190 }}>
  ...
  return (
    <Box sx={{ p: 4 }}>
      ...
      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap", mb: 3 }}>
        <MetricCard label="Oficinas ativas" value={metricas?.total_oficinas_ativas} icon={<BusinessRoundedIcon fontSize="small" />} loading={loading} />
        <MetricCard label="Oficinas inativas" value={metricas?.total_oficinas_inativas} icon={<BusinessRoundedIcon fontSize="small" />} loading={loading} />
        <MetricCard label="Novas este mês" value={metricas?.oficinas_no_mes} icon={<TrendingUpRoundedIcon fontSize="small" />} loading={loading} sublabel={`${metricas?.oficinas_30_dias ?? 0} nos últimos 30 dias`} />
        <MetricCard label="Usuários" value={metricas?.total_usuarios} icon={<PeopleRoundedIcon fontSize="small" />} loading={loading} />
        <MetricCard label="OSs em aberto" value={metricas?.total_os_abertas} icon={<BuildRoundedIcon fontSize="small" />} loading={loading} sublabel={`${metricas?.oficinas_com_os_aberta ?? 0} oficinas com OS aberta`} />
      </Box>
